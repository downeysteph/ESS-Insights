...
> 🆕 **Why 1.6 matters for refresh:** Earlier template versions could trip a **"dynamic data source"** error in the service that blocked *all* scheduled refresh. Version **1.6** restructures every data source (Dataverse and web/SharePoint) so the service can identify and authenticate it statically — the prerequisite for scheduled refresh. **Use the 1.6 templates** for anything you intend to auto‑refresh.
...
1. **Publish** the `ESS Dashboard Template 1.6 (Dataverse)` report to your workspace.
...
3. Open `ESS Dashboard Template 1.6 (CSV Upload)` in Desktop. When prompted (or via **Transform data → Edit Parameters**), paste the **direct SharePoint URL** into **Copilot Studio Transcript**. (Optional Org Data / Agent Credits files can be hosted the same way.)
...
|---|---|---|
| **“You can't schedule refresh for this dataset because … the data sources are dynamic.”** | Data source URL/connection can't be resolved statically (older template, or a viewer‑style SharePoint link). | Use the **1.6** template, and for CSV use a **direct file path URL** (Path B), not a `?web=1`/sharing link. |
