...
> 🆕 **Why v1.8 matters for refresh:** Earlier template versions could trip a **"dynamic data source"** error in the service that blocked *all* scheduled refresh. Version **1.8** restructures every data source (Dataverse and web/SharePoint) so the service can identify and authenticate it statically — the prerequisite for scheduled refresh. **Use the 1.8 templates** for anything you intend to auto‑refresh.
...
|---|---|---|
| **“You can't schedule refresh for this dataset because … the data sources are dynamic.”** | Data source URL/connection can't be resolved statically (older template, or a viewer‑style SharePoint link). | Use the **1.8** template, and for CSV use a **direct file path URL** (Path B), not a `?web=1`/sharing link. |
